...
app.get("/api/ping", (request, response) => {
  console.log(
    moment(Date.now())
      ...
      .format("DD/MM/YYY HH:mm:ss") + " Ping Received"
  );
  response.status(200).send(moment(Date.now()).format() + " Ping Received");
});
setInterval(() => {
  ...
  switch (req.params.name) {
      ...
      try {
        if (req.query && req.query.url) {
          ...
          function blur(strength) {
            ...
            for (var y = -strength; y <= strength; y += 2) {
              for (var x = -strength; x <= strength; x += 2) {
                ...
                if (x >= 0 && y >= 0) ctx.drawImage(canvas, -(x - 3), -(y - 1));
              }
            }
          ...
          if (!req.query || !req.query.url) {
            res.status(400).json({
              message:
                "Bad Request, you must specify the 'url' query parameter.",
              status: 400
            });
          }
        }
    ...
    case "triggered": {
      ...
      try {
        ...
        avatar.color([{ apply: "mix", params: ["#FF0000", "30"] }]);
        ...
        res.status(500).json({
          message:
            "Unexpected error. Post an issue on github or contact me (KingBR#3793) on discord",
          status: 500
        });
      }
      break;
